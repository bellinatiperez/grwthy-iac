...
Send messages through the WhatsApp Cloud API. All messages are processed asynchronously with automatic retries for reliable delivery.
...
All send endpoints return the same response:
...
Track delivery status via [webhook events](/webhooks/events).
